...
| JSON breadth | The self-host now supports tuples, recursively serializable `Map<string, T>` fields, fixed class fields, and closed-world interface deserialization through a shared fixed string discriminator. Named union-alias and general non-null union dispatch remain missing. `JsonSerializable` generic access exists, but it inherits this narrower concrete dispatch surface. | P1 | Run a shared JSON corpus for tuples, nested collections, interfaces, aliases resolving to dispatchable types, recursive values, nullable values, strict/lenient conversion, defaults, and path-preserving failures. |
