...
| External dependencies | Archive/Git external dependencies, integrity/ref pinning, setup commands, sentinels, and target interpolation are reference-only. | `DoofExternalDependencyConfig` and acquisition flow in `src/package-manifest.ts` have no self-host counterpart. | P1 | Port the reference external-dependency fixtures and compare the materialized native plan and provenance. |
