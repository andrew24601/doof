...
describe("emitter — namespace imports", () => {
  ...
  it("wraps main(args) using the Doof array representation", () => {
    ...
    `);
    expect(cppCode).toContain("auto args = argc > 1 && argv != nullptr");
    expect(cppCode).toContain("? std::make_shared<std::vector<std::string>>(argv + 1, argv + argc)");
    expect(cppCode).toContain(": std::make_shared<std::vector<std::string>>();");
